...
def database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `enter` (fname TEXT, lname TEXT, mid TEXT, username TEXT, password TEXT, confirm_password TEXT)")

    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `enter` VALUES(?,?,?,?,?,?)",(FNAME.get(), LNAME.get(), MID.get(), USERNAME.get(), PASSWORD.get(), CONFIRM_PASSWORD.get()))
        conn.commit()
